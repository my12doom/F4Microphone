...
int do_tx(void *pdev)
{
	...
	if (buffer_ready &1) {
		...
		memcpy(tx_buf, adc_data_out, AUDIO_IN_PACKET);
		DCD_EP_Tx (pdev,AUDIO_IN_EP, (uint8_t*)tx_buf, AUDIO_IN_PACKET);//length in words to bytes
		//DCD_EP_Tx (pdev,AUDIO_IN_EP, (uint8_t*)adc_data_out, AUDIO_IN_PACKET);//length in words to bytes
	...
	if (buffer_ready &2) {
		usb_idle = 0;
		DCD_EP_Flush(pdev,AUDIO_IN_EP);//very important!!!
		memcpy(tx_buf, adc_data_out+ADC_BUFFER_SIZE/2, AUDIO_IN_PACKET);
		DCD_EP_Tx (pdev,AUDIO_IN_EP, (uint8_t*)tx_buf, AUDIO_IN_PACKET);//length in words to bytes
		//DCD_EP_Tx (pdev,AUDIO_IN_EP, (uint8_t*)(adc_data+ADC_BUFFER_SIZE/2), AUDIO_IN_PACKET);//length in words to bytes
		buffer_ready &= ~2;
	}
	*/
